...
namespace SFA.DAS.ApprenticeAccounts.UnitTests.Infrastructure.MediatorTests
{
    public class WhenLoggingTheMediatorPipeline
    {
        ...
        {
            var response = await _sut.Handle(request, _ => Task.FromResult(expectedResponse), CancellationToken.None);
        ...
        {
            await _sut.Handle(request, _ => Task.FromResult(expectedResponse), CancellationToken.None);
        ...
        {
            Func<Task> action = () => _sut.Handle(request, _ => throw new Exception("failed"), CancellationToken.None);
